...
bool sensorsAutodetect(sensorAlignmentConfig_t *sensorAlignmentConfig, uint16_t gyroLpf, uint8_t accHardwareToUse, uint8_t magHardwareToUse, uint8_t baroHardwareToUse, int16_t magDeclinationFromConfig, uint32_t looptime, uint8_t syncGyroToLoop, uint8_t syncGyroToLoopDenom)
{
    int16_t deg, min;
    ...
    gyroUpdateSampleRate(looptime, gyroLpf, syncGyroToLoop, syncGyroToLoopDenom);      // Set gyro refresh rate before initialisation
    gyro.init(gyroLpf);
